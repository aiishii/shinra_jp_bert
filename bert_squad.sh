...
for target in ${categories[@]}; do
    echo $target
    # python berqt_squad.py --do_train --category ${target} --per_gpu_train_batch_size ${batch_size} --per_gpu_eval_batch_size ${eval_batch_size} --learning_rate ${LR} --max_seq_length ${max_seq_length} --doc_stride ${doc_stride} --test_case_str ${test_case_str}_${GROUP} --data_dir ${work_dir} --model_name_or_path ${best_model_path}
    # BEST_EPOCH=${?}
    BEST_EPOCH=9
    ...
    # python regulation_bio.py --predicate_json ${model_dir}/epoch-${BEST_EPOCH}/shinra_${TARGET}_test_results.json --category ${TARGET} --html_dir ${data_dir}/html/${TARGET} --prefix ${prefix} --dist_file ${data_dir}/annotation/${TARGET}_dist.json
    # cd ..
